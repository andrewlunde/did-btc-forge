...
if (process.env.BTC_RPC_NETWORK === undefined) {
    console.log("Missing BTC_RPC_NETWORK defaulting to mainnet");
    btc_rpc_network = 'mainnet';
}
else {
    btc_rpc_network = process.env.BTC_RPC_NETWORK;
}
if (process.env.BTC_RPC_USER === undefined) {
    console.log("Missing BTC_RPC_USER");
}
else {
    btc_rpc_user = process.env.BTC_RPC_USER;
}
if (process.env.BTC_RPC_PASSWORD === undefined) {
    console.log("Missing BTC_RPC_PASSWORD");
}
else {
    btc_rpc_password = process.env.BTC_RPC_PASSWORD;
}
if (process.env.BTC_RPC_HOST === undefined) {
    console.log("Missing BTC_RPC_HOST");
}
else {
    btc_rpc_host = process.env.BTC_RPC_HOST;
}
if (process.env.BTC_RPC_PORT === undefined) {
    console.log("Missing BTC_RPC_PORT");
}
else {
    btc_rpc_port = process.env.BTC_RPC_PORT;
}
//Define the network
...
async function getJSONconfig(file_name) {
    try {
...
async function setJSONconfig(message) {
    var result = false;
    try {
...
const GetTransactionAtIndex = async (hash, txidx) => {
    const body = {
        jsonrpc: "1.0",
        id: "curltest",
        method: "getblock",
...
const GetUTXOs4addr = async (hash, fund_addr) => {
    const body = {
        jsonrpc: "1.0",
        id: "curltest",
        method: "getblock",
...
const scan4utxos = async (funding_addr) => {
...
const wait4funds = async (funding_addr, satoshis_needed) => {
...
const GetFeeRateEst = async (target_blocks) => {
    const body = {
        jsonrpc: "1.0",
        id: "curltest",
...
;
const GetFeeEstimates = async (target_blocks) => {
        ...
        case STAGE_ENUM.VERIFY_DID:
            ...
            if (typeof didIndex !== 'undefined') {
                ...
                if (typeof decodedDidId.didIndex !== 'undefined') {
                    const did = resolveDidBtc([tx.hex], decodedDidId.didIndex);
